...
```{r exp_setup}
run.adv=FALSE #run advance model 
run.cv=TRUE # run cross-validation on the training set
k <- 5  # number of CV folds
seed <- 123  #random seed
train_p <- 0.8 #the proportion of training data
run.feature.train=FALSE # process RGB features for training set
...
if (!run.adv){
  ...
  tm_test<-system.time(pred.new<-predict(opt,as.matrix(new.data)))
}
