...
surf.train<-read.csv('output/SURF_train.csv',header=F,sep=',')
surf.train<-surf.train[-1,]
surf.test<-read.csv('output/SURF_test.csv',header=F,sep=',')
surf.test<-surf.test[-1,]
train.label<-read.csv('output/train_label_surf.csv',header=T,sep=',')
test.label<-read.csv('output/test_label_surf.csv',header=T,sep=',')
train.y<-train.label[,1]-1
test.y<-test.label[,1]-1
train.data<-cbind(y=train.y,surf.train)
test.data<-cbind(y=test.y,surf.test)
